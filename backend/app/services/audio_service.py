...
class AudioService:
    ...
    async def upload_audio(self, file: UploadFile, user: User) -> FileResponse:
        ...
        storage.upload_fileobj(io.BytesIO(content), key, file.content_type)
        ...
        file_url = storage.generate_presigned_get(key, ttl=LIST_PRESIGN_TTL)
    ...
    async def list_audio(self, user: User, limit: int, offset: int) -> dict:
        ...
        storage = self._storage()
        items = [
            _to_response(f, storage.generate_presigned_get(f.key, ttl=LIST_PRESIGN_TTL))
            for f in files
        ]
